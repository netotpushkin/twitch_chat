...
Почему base64 в SSE, а не отдельный HTTP GET за WAV:
    ...
    goal/images) держат эти слоты 24/7 — любой GET /tts/audio/<id> встаёт в очередь
    и никогда не выполняется. Инлайн WAV в само событие обходит проблему: лишних
    коннектов не нужно. (Медиа-оверлей по той же причине ушёл на WebSocket /ws/media
    — у WS отдельный, более щедрый лимит, и SSE-пул он не занимает.)
...
# Короткие чат-реплики ("+", "лол", "спс", "ок") не озвучиваем — на 100мс
# fade-in/out от них ничего не остаётся, и в звуковом потоке это просто щелчки.
# Озвучиваем сообщения от _CHAT_MIN_CHARS символов (после очистки).
