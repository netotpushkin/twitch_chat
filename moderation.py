...
Точка входа — moderate(tags, login, text), её зовёт IRC-loop в bot.py. Привилегии
проверяются сразу в moderate() (fast-path, без пула); остальное идёт в фоновый пул
потоков, чтобы не тормозить приём сообщений.

Пайплайн:
  1. Привилегии (broadcaster/mod/vip) → пропуск (ещё в moderate, без пула).
  2. Известная бот-команда (!ютуб и т.п.) → пропуск.
  3. ASCII-арт / символьный спам → DELETE без LLM.
  4. URL не на YouTube → DELETE без LLM (текст нормализуется один раз и далее
     переиспользуется в проверке длины и вводе LLM).
  5. Мало «полезных» символов (букв/цифр) → пропуск, в LLM не гоняем.
  6. LLM-вердикт по тексту без ссылок (с LRU-кэшем по тексту): темы не ограничены —
     режем только переход черты по «градусу» (реальные призывы к вреду,
     расчеловечивание/ненависть, дети, суицид), а мат, оскорбления, грубые мнения
     и споры на любую тему — пропускаем.
...
def _llm_verdict(text):
    ...
    try:
        reply = llm_ask(
            ...
            # Routing по цене (самый дешёвый провайдер) берётся из дефолта openrouter —
            # явно передавать provider не нужно.
        )
