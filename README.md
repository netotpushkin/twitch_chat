...
Самописный чат-бот для Twitch с набором интеграций: модерация через LLM, авто-подбор заголовка и тегов стрима, очередь YouTube-клипов с голосованием, бросок кубика «гейммастер», TTS донатов и «короля доната», приём донатов через Donatty, EventSub-алерты и набор HTML-оверлеев для OBS.
...
- **YouTube-очередь** (`youtube.py`): проверка ролика скрейпом watch-страницы (`ytInitialPlayerResponse`: статус/длина/просмотры/embeddable), голосование за скип с окном `YT_VOTE_WINDOW`, фоновое выполнение в пуле.
...
- **Картинки на экране** (`image_bus` / `images.html`): прямые ссылки на картинки/гифки/webm от вип и модов выводятся на оверлей небольшим размером в случайном месте (картинки — 5 с, webm — на длину клипа, ≤15 с, со звуком). Вес ≤3 МБ, проверка `Content-Type` и только внешние адреса (HEAD, без редиректов). Сообщение с такой ссылкой не дублируется в чат-оверлей.
...
| `state/` | Рантайм-стейт (token.json, overlay_token.txt, ...). Не в git. |
...
| `/images.html` | Картинки/гифки/webm от вип и модов (по ссылке в чате). |
