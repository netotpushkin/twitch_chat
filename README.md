...
- **Команды чата** (`commands.py`):
  - `!ютуб <url>` — добавить YouTube-ролик в очередь (1–8 мин, 10000+ просмотров, не Shorts, не трансляция);
  ...
  - `!озвучка` — переключить TTS-режим: только король доната ↔ все сообщения чата (только моды/стример);
  - `!громкость <0-100>` — выставить громкость YouTube-оверлея (без аргумента показывает текущую, только моды/стример).
...
- Кубик-оверлей + веб-камера + индекс оверлеев.
...
| `overlays/*.html` | OBS Browser-source оверлеи (chat, alerts, dice, donatty, goal, images, webcam, youtube). |
